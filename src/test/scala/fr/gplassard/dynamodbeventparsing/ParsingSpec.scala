...
class ParsingSpec extends Specification with Rules {
  ...
  "The int rule" should {
    "extract from a string attribute" in {
      val attribute = new AttributeValue().withN("12")

      intRule.validate(attribute) must beEqualTo(Valid(12))
    }

    "fail if it's not an int" in {
      val attribute = new AttributeValue().withN("12.3")
  ...
  "The person rule" should {
    "extract the case class" in {
      val dynamoDocument = Map("name" -> new AttributeValue("John Doe"), "age" -> new AttributeValue().withN("42"), "lovesChocolate" -> new AttributeValue().withBOOL(true)).asJava
    ...
    "fail if fields are incorrect" in {
      val dynamoDocument = Map("name" -> new AttributeValue("John Doe"), "age" -> new AttributeValue().withN("42.5"), "lovesChocolate" -> new AttributeValue().withBOOL(true)).asJava

      personRule.validate(dynamoDocument).isInvalid must beTrue
    }

    "fail if fields are missing" in {
      val dynamoDocument = Map("name" -> new AttributeValue("John Doe"), "age" -> new AttributeValue().withN("42")).asJava
